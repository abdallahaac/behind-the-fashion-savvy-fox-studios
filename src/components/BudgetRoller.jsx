...
const DigitRoller = ({ digit, rolling, delay }) => {
    ...
    return (
        <div className="digit">
            <div className="digit-value accent-1">{isNaN(currentDigit) ? '-' : currentDigit}</div>
        </div>
    );
};
...
const BudgetRoller = ({ onRollDone }) => {
    ...
    const numberString = number !== null ? number.toString().padStart(6, '0') : '000000';
    const numberWithComma = `${numberString.slice(0, 3)},${numberString.slice(3)}`;
        ...
        <div className="budget-roller">
            <div className="number-display">
                ...
                {numberWithComma.split('').map((char, index) => (
                    char === ',' ? (
                        <div key={index} className="comma accent-1">,</div>
                    ) : (
                        <DigitRoller key={index} digit={char} rolling={rolling} delay={index * 100} />
                    )
                ))}
            </div>
            <button
