export const handleNext = (mode, setMode, questionIndex, setQuestionIndex, selectedQuestions, setCurrentStep, setResult, setFunding) => {
    if (mode === "Normal") {
        setMode("Quiz");
        setCurrentStep((prev) => prev + 1);
    } else if (mode === "Quiz") {
        const selectedAnswer = document.querySelector('input[name="answer"]:checked')?.nextSibling.textContent;
        if (!selectedAnswer) {
            alert("Please select an answer before proceeding.");
            return;
        }
        setMode("Result");
        // Calculate the result based on the selected answer
